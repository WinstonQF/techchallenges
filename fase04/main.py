...
#######################################
N_FRAMES_HISTORY = 30  # Quantos frames de histórico para suavizar a detecção de atividade
...
def create_video_writer(cap, output_video_path):
    """
    Cria um VideoWriter para salvar o vídeo de saída.
    """
...
def get_or_create_individual_record(face_encoding, known_individuals, tolerance=0.8):
    """
    Dado um face_encoding, verifica se é um indivíduo conhecido ou cria um novo registro
    ...
    # Ajuste de margem: se a distância mínima for maior que o limite definido,
    # consideramos o rosto como não correspondente a nenhum registro conhecido.
    # MARGIN_THRESHOLD = 0.7  # Valor ajustável para maior ou menor sensibilidade
    # if distances[best_match_idx] > MARGIN_THRESHOLD:
    #    return create_new_individual_record(face_encoding, known_individuals)
...
def analyze_emotion(face_roi):
    ...
    Esse código permite identificação da emoção de um indivíduo deitado
    ou de lado, cado da mulher deitada.    
...
def show_summary(total_frames, known_individuals):
    ...
    for ind in known_individuals:
        ...
        print(f"   Atividade mais frequente: {ind['last_activity']}")
...
if __name__ == "__main__":
    # Altere os caminhos
    input_video = "dados/in/sample_individuals.mp4"
    output_video = "dados/out/sample_individuals_analisado.mp4"
